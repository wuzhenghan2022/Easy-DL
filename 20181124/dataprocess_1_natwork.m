...
% 
% 
% image=[r12a,r13a,r12a,r1a,r2a,r3a,r4a,r5a,r8a,r10a,r11a];
...
%%
    ZSbg=textread("testdataresult.txt");
...
imagedata=textread("testimage.txt");
SDB_Stumpf_bg=reshape(imagedata,376,391);
csv1=csvread("image.csv");
lon1=csv1(:,1);
lat1=csv1(:,2);
lon=reshape(lon1,376,391);
lat=reshape(lat1,376,391);


SS = shaperead('vskp_watermask.shp');
isin = inpolygon(lon,lat,SS.X,SS.Y);
load ZLm.mat;
% 使用isnan函数检测ZLm中的NaN值，并获取相应的索引  
nan_indices = isnan(ZLm);  
  
% 使用索引将NaN值赋值给SDB_Stumpf_bg的相应位置  
SDB_Stumpf_bg(nan_indices) = NaN;  
...
clear X
    ...
    sdb11=textread("testdataresult.txt");
    csv=csvread("testdata_yanzheng.csv");
    dept1=csv(2:999,3);
    disp('For network...')
    val1 = dept1;
