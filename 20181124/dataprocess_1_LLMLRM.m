...
fname = 'S2B_MSI_2018_11_24_05_03_51_T44QQE_L2W.nc';
...
image=[r12a,r13a,r12a,r1a,r2a,r3a,r4a,r5a,r6a,r8a,r10a,r11a];
image(isnan(image)==1) = 0;
writematrix(image,'image.csv')
...
% depth=csv;
% [m,n] = size(depth) ;
% P = 0.1 ; % percentage of training dataset
...
writematrix(data,'testdata_yanzheng2.csv')
...
% r1011=csv(:,11);
% % r11=csv(:,11);
% XI = log(r111-min(min(r111)));
% XJ = log(r211-min(min(r211)));
% XK = log(r311-min(min(r311)));
% XL = log(r411-min(min(r411)));
% XM = log(r511-min(min(r511)));
% XN = log(r611-min(min(r611)));
% XO = log(r811-min(min(r811)));
% XP = log(r1011-min(min(r1011)));
...
depthx=val1;
H=val2;
 jiguang=depthx(depthx<3);
xielv=H(depthx<3);
R=corrcoef(jiguang,xielv);
R21=R(1,2)*R(1,2);
[n1,~]=size(jiguang);
MAE1=sum(abs(jiguang-xielv))/n1;
MRE1=(100*sum(abs((jiguang-xielv)./jiguang)))/n1;
RMSE1=sqrt((sum((jiguang-xielv).*(jiguang-xielv))/n1));
%%
 jiguang=depthx(depthx>3&depthx<6);
xielv=H(depthx>3&depthx<6);
R=corrcoef(jiguang,xielv);
R22=R(1,2)*R(1,2);
[n2,~]=size(jiguang);
MAE2=sum(abs(jiguang-xielv))/n2;
MRE2=(100*sum(abs((jiguang-xielv)./jiguang)))/n2;
RMSE2=sqrt((sum((jiguang-xielv).*(jiguang-xielv))/n2));
%%
 jiguang=depthx(depthx>6&depthx<9);
xielv=H(depthx>6&depthx<9);
R=corrcoef(jiguang,xielv);
R23=R(1,2)*R(1,2);
[n3,~]=size(jiguang);
MAE3=sum(abs(jiguang-xielv))/n3;
MRE3=(100*sum(abs((jiguang-xielv)./jiguang)))/n3;
RMSE3=sqrt((sum((jiguang-xielv).*(jiguang-xielv))/n3));


%%
 jiguang=depthx(depthx>9);
xielv=H(depthx>9);
R=corrcoef(jiguang,xielv);
R24=R(1,2)*R(1,2);
[n4,~]=size(jiguang);
MAE4=sum(abs(jiguang-xielv))/n4;
MRE4=(100*sum(abs((jiguang-xielv)./jiguang)))/n4;
RMSE4=sqrt((sum((jiguang-xielv).*(jiguang-xielv))/n4));
...
csv = csvread('testdata_yanzheng.csv');
lt=csv(:,2);
ln=csv(:,1);
dept=csv(:,3);
...
ylabel('SDB OBRA (m)','FontSize',18,'Fontweight','bold')
dat1 = [num2str(dat(1:4)),'-',num2str(dat(6:7)),'-',num2str(dat(9:10))];
title("OBRA Model",'fontsize',20);
set(gca,'FontSize',20)
...
clear X
    ...
    data1=textread("testdataresult.txt");
    csv=csvread("testdata_yanzheng.csv");
    csv1=csv(2:999,3);
    disp('For network...')

 f1 = fit(data1,csv1,'poly1');
ci = confint(f1);
...
figure(71)
histogram(diff1,'FaceColor',[0 0 0.8])
hold on
histogram(diff2,'FaceColor',[0.8 0 0])
hold on
histogram(diff3,'FaceColor',[0 0.8 0])
legend('Stumpf Model','Log-Linear Model','Easy-DL Model')
xlim([-10 10])
...
depthx=val3;
H=val1;
 jiguang=jiguang(depthx<3);
xielv=gaoguangpu(depthx<3);
R=corrcoef(jiguang,xielv);
R21=R(1,2)*R(1,2);
[n1,~]=size(jiguang);
MAE1=sum(abs(jiguang-xielv))/n1;
